    ...
    try:
      ...
      if serialized.is_valid():
        ...
        request_data = {
          ...
          'user': User.objects.get(pk=serialized.data.get('user')) if serialized.data.get('user', None) else None,
          'system_specs': SystemSpecs.objects.get(pk=serialized.data.get('user')) if serialized.data.get('system_specs',
                                                                                                         None) else None,
        }
        ...
        if serialized.data.get('server_key', None):
          ...
          for item in serialized.data.get('server_key'):
            obj = ServerUserKey(server=server_object, user=user, keypair=KeyPair.objects.get(pk=item))
    ...
    if kwargs.get("pk"):
      try:
        ...
        response = {
          ...
          'server_user_keys': ServerUserKeySerializer(ServerUserKey.objects.filter(server=kwargs.get('pk'))).data
        }
        return Response(response, status=status.HTTP_200_OK)
      except Server.DoesNotExist:
        return Response(
          "Server Not Found", status=status.HTTP_404_NOT_FOUND
        )
